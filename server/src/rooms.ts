...
function hiddenTiles(count: number) {
...
export function startGame(code: string, socketId: string): Room {
  ...
  if (room.state && !room.state.gameOver) {
    throw new Error("Game is already started. Use next hand to continue.");
  }

  // Create fresh game state (either first start or a new game after game over)
