...
function roomWithPlayers() {
  const room = rooms.createRoom(`host-${crypto.randomUUID()}`);
  rooms.joinRoom(room.code, `guest-${crypto.randomUUID()}`);
  return room;
}
...
describe("room lifecycle authorization", () => {
  it("rejects game start from a socket that is not seated in the room", () => {
    ...
    expect(() => rooms.startGame(room.code, `intruder-${crypto.randomUUID()}`)).toThrow(
      "not a player"
    );
  });
  ...
  it("rejects next hand from a socket that is not seated in the room", () => {
    ...
    expect(() => rooms.nextHand(room.code, `intruder-${crypto.randomUUID()}`)).toThrow(
      "not a player"
    );
  });
  ...
  it("does not allow game:start to reset scores between hands", () => {
    ...
    expect(() => rooms.startGame(room.code, playerId)).toThrow("Game is already in progress");
...
describe("visible game state", () => {
  it("hides opponent hands, boneyard tiles, and dead tiles from each player", () => {
    ...
    const getVisibleGameState = (rooms as any).getVisibleGameState;

    expect(typeof getVisibleGameState).toBe("function");
    const visible = getVisibleGameState(room.state, viewerId) as GameState;

    expect(visible.players[viewerId].hand).toEqual(room.state.players[viewerId].hand);
    expect(visible.players[opponentId].hand).toHaveLength(room.state.players[opponentId].hand.length);
    expect(visible.players[opponentId].hand.every(tile => tile.high === -1 && tile.low === -1)).toBe(true);
