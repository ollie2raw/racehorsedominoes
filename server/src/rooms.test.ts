...
describe("room lifecycle authorization", () => {
  ...
  it("rejects game start between hands so accumulated scores are not wiped", () => {
    ...
    expect(() => startGame(room.code, "player-a")).toThrow(/already started|next hand/i);
...
describe("visible room state", () => {
  it("only reveals the recipient player's hand and masks hidden tiles", () => {
    ...
    expect(visibleForA.players["player-a"].hand).toEqual(rawState.players["player-a"].hand);
    expect(visibleForA.players["player-b"].hand).toHaveLength(rawState.players["player-b"].hand.length);
    expect(visibleForA.players["player-b"].hand.every(tile => tile.high === -1 && tile.low === -1)).toBe(true);
    expect(visibleForA.boneyard).toHaveLength(rawState.boneyard.length);
    expect(visibleForA.boneyard.every(tile => tile.high === -1 && tile.low === -1)).toBe(true);
    expect(visibleForA.deadTiles).toHaveLength(rawState.deadTiles.length);
    expect(visibleForA.deadTiles.every(tile => tile.high === -1 && tile.low === -1)).toBe(true);
  });
});
