...
const rooms = new Map();
function maskTiles(count) {
    return Array.from({ length: count }, () => ({ high: -1, low: -1 }));
}
function assertRoomPlayer(room, socketId) {
    if (!room.players.includes(socketId)) {
        throw new Error("You are not a player in this room.");
    }
}
function getVisibleGameState(state, viewerId) {
    const players = Object.fromEntries(state.playerIds.map(id => {
        const player = state.players[id];
        return [
            id,
            {
                ...player,
                hand: id === viewerId ? [...player.hand] : maskTiles(player.hand.length),
            },
        ];
    }));
    return {
        ...state,
        players,
        boneyard: maskTiles(state.boneyard.length),
        deadTiles: maskTiles(state.deadTiles.length),
    };
}
...
function startGame(code, socketId) {
    ...
    if (room.state && !room.state.gameOver) {
        if (room.state.handOver) {
            throw new Error("Hand is over. Call next hand to continue.");
        }
        throw new Error("Game is already in progress.");
    }
    // Create fresh game state for first start, or for a new match after game over.
