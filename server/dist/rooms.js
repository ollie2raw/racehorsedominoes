...
function assertPlayerInRoom(room, socketId) {
    if (!room.players.includes(socketId)) {
...
function getVisibleGameState(state, viewerSocketId) {
    const players = Object.fromEntries(Object.entries(state.players).map(([id, player]) => [
        ...
        {
            ...player,
            hand: id === viewerSocketId ? player.hand : hideTiles(player.hand),
        },
    ]));
    return {
        ...state,
...
function startGame(code, socketId) {
    ...
    assertPlayerInRoom(room, socketId);
    if (room.players.length !== 2) {
    ...
    // Create fresh game state (either first start or new game after completion)
