...
function getVisibleGameState(state, viewerId) {
    return {
        ...
        players: Object.fromEntries(Object.entries(state.players).map(([playerId, player]) => [
            ...
            {
                ...player,
                hand: playerId === viewerId ? player.hand : player.hand.map(() => HIDDEN_TILE),
            },
        ])),
        boneyard: state.boneyard.map(() => HIDDEN_TILE),
        deadTiles: state.deadTiles.map(() => HIDDEN_TILE),
    };
}
function startGame(code, socketId) {
    ...
    // Create fresh game state (first start, or a new game after game over).
